...
class Window_W1(QMainWindow):
    def __init__(self,parent=None):
        ...
        self.l_2.setText("Имя")
        
        self.l_3 = QtWidgets.QLabel(self)
        self.l_3.move(220,255)
        self.l_3.setText("Фамилия")
        
        self.l_4 = QtWidgets.QLabel(self)
        self.l_4.move(320,255)
        self.l_4.setText("Aдрес")

        self.l_4 = QtWidgets.QLabel(self)
        self.l_4.move(420,255)
        self.l_4.setText("Дата рождения")

        self.l_4 = QtWidgets.QLabel(self)
        self.l_4.move(520,255)
        self.l_4.setText("Телефон")
    ...
    def up(self):
        ...
        self.table.setFixedSize(770, 250)
        # Устанавливаем заголовки
        self.c.execute('SELECT W_Number,W_Name,W_Surname,W_address,W_data,W_PhoneNumber FROM Workers')
        rows = self.c.fetchall()
        # Устанавливаем количество строк и столбцов
        self.table.setHorizontalHeaderLabels(['№', 'Имя', 'Фамилия','Адрес','ДР','Телефон'])
        for row_number, row_data in enumerate(rows):
            self.table.insertRow(row_number)
        for column_number, data in enumerate(row_data):
            self.table.setItem(row_number, column_number,QtWidgets.QTableWidgetItem(str(data)))
            self.setCentralWidget(self.table)
        self.query = f"SELECT * FROM Workers WHERE W_Number LIKE '%{Number}%';"
    ...
    def search(self):
        ...
        self.table.setColumnCount(6)
        self.table.setRowCount(0)
        # Устанавливаем заголовки W_Number,W_Name,W_Surname,W_address,W_data,W_PhoneNumber
        self.c.execute("SELECT * FROM Workers WHERE W_Number LIKE ? OR W_Name LIKE ? OR W_Surname LIKE ? OR W_address LIKE ? OR W_data LIKE ? OR W_PhoneNumber LIKE ?;", ('%' + Number + '%','%' + Number + '%','%' + Number + '%','%' + Number + '%','%' + Number + '%','%' + Number + '%',))
        rows = self.c.fetchall()
        # Устанавливаем количество строк и столбцов
        self.table.setHorizontalHeaderLabels(['№', 'Имя', 'Фамилия','Адрес','ДР','Телефон'])
        for row_number, row_data in enumerate(rows):
    ...
    def updata(self):
        ...
        Name = self.T_B2.text()
        Surname = self.T_B3.text()
        Address = self.T_B4.text()
        Data = self.T_B5.text()
        PhoneNumber = self.T_B6.text()
        if len(Name) > 0:
            self.c.execute("UPDATE Workers SET W_Name=? WHERE W_Number=?", (Name, Number))
        if len(Surname) > 0:
            self.c.execute("UPDATE Workers SET W_Surname=? WHERE W_Number=?", (Surname, Number))
        if len(Address) > 0:
            self.c.execute("UPDATE Workers SET W_Address=? WHERE W_Number=?", (Address, Number))
        if len(Data) > 0:
            self.c.execute("UPDATE Workers SET W_data=? WHERE W_Number=?", (Data, Number))
        if len(PhoneNumber) > 0:
            self.c.execute("UPDATE Workers SET W_PhoneNumber=? WHERE W_Number=?", (PhoneNumber, Number))
        self.conn.commit()
        self.up()
    ...
    def delete(self):
        ...
        if i == 1:
            ...
            # формирование и выполнение SQL запроса
                query = "DELETE FROM Workers WHERE W_Number=?"
    ...
    def add(self):
        ...
        PhoneNumber = self.T_B6.text()
        # формирование и выполнение SQL запроса
        query = "INSERT INTO Workers(W_Name,W_Surname,W_address,W_data,W_PhoneNumber) VALUES (?, ?, ?, ?, ?)"
        self.c.execute(query, (Name,Surname,Address,Data,PhoneNumber))
        self.conn.commit()
        self.up()
        if self.rb1.isChecked():
            self.clear()
...
def word():
        ...
        document.save('Сотрудники.docx')
        filename = 'Сотрудники.docx'
