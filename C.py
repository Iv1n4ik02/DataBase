...
class Window_C(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Таблица Клиенты")
        ...
        self.Updata_B.move(220,360)
        self.Updata_B.setText("Очистка")
        self.Updata_B.clicked.connect(self.clear)
    ...
    def up(self):
        ...
        self.c.execute('SELECT C_Number, C_Name,C_Surname,C_PhoneNumber FROM Client')
        ...
        self.query = f"SELECT * FROM Client WHERE C_Number LIKE '%{Number}%' OR C_Name LIKE '%{Name}%' OR C_Surname LIKE '%{Surname}%' OR C_PhoneNumber LIKE '%{PhoneNumber}%';"
    ...
    def search(self):
        ...
        self.table.setRowCount(0)
        # Устанавливаем заголовки
        self.c.execute("SELECT * FROM Client WHERE C_Number LIKE ? OR C_Name LIKE ? OR C_Surname LIKE ? OR C_PhoneNumber LIKE ?", ('%' + Number + '%','%' + Number + '%','%' + Number + '%','%' + Number + '%',))
        rows = self.c.fetchall()
        # Устанавливаем количество строк и столбцов
        self.table.setHorizontalHeaderLabels(['№', 'Имя', 'Фамилия','Телефон'])
        for row_number, row_data in enumerate(rows):
    ...
    def updata(self):
        ...
        PhoneNumber = self.T_B4.text()
        if len(Name) > 0:
            self.c.execute("UPDATE Client SET C_Name=? WHERE C_Number=?", (Name, Number))
        if len(Surname) > 0:
            self.c.execute("UPDATE Client SET C_Surname=? WHERE C_Number=?", (Surname, Number))
        if len(PhoneNumber) > 0:
            self.c.execute("UPDATE Client SET C_PhoneNumber=? WHERE C_Number=?", (PhoneNumber, Number))
        self.conn.commit()
        self.up()
    ...
    def delete(self):
        ...
        if i == 1:
            ...
            # формирование и выполнение SQL запроса
                query = "DELETE FROM Client WHERE C_Number=?"
    ...
    def add(self):
        ...
        PhoneNumber = self.T_B4.text()
        # формирование и выполнение SQL запроса
        query = "INSERT INTO Client(C_Name,C_Surname,C_PhoneNumber) VALUES (?, ?, ?)"
        self.c.execute(query, (Name,Surname,PhoneNumber))
        self.conn.commit()
        self.up()
        if self.rb1.isChecked():
            self.clear()
...
def word():
    ...
    document.save('Клиенты.docx')
    filename = 'Клиенты.docx'
