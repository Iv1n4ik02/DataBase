...
from PyQt5.QtWidgets import QApplication, QWidget,QPushButton, QVBoxLayout, QHBoxLayout,QMainWindow,QRadioButton,QMessageBox
from openpyxl.workbook import Workbook
from PyQt5.QtCore import Qt
from PyQt5.QtGui import QIcon

class Window_P(QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.conn = sqlite3.connect('DataBase.db')
        self.c = self.conn.cursor()
        self.c.execute('SELECT * FROM Pantry')
        super(Window_P,self).__init__()
        self.setWindowTitle("Таблица Склад")
        self.setGeometry(640,320,600,400)
        
        self.Exit_B = QtWidgets.QPushButton(self)
        self.Exit_B.move(530,360)
        ...
        self.Exel_B = QtWidgets.QPushButton(self)
        self.Exel_B.move(420,360)
        self.Exel_B.setText("+1")
        self.Exel_B.clicked.connect(self.Up1)

        self.Exel_B = QtWidgets.QPushButton(self)
        self.Exel_B.move(320,360)
        self.Exel_B.setText("-1")
        self.Exel_B.clicked.connect(self.Down1)

        self.Color_B = QtWidgets.QPushButton(self)
        self.Color_B.move(530,320)
    ...
    def up(self):
        ...
        self.c.execute('SELECT P_Number, P_Name,P_Type,P_des,P_qua FROM Pantry')
        rows = self.c.fetchall()
        # Устанавливаем количество строк и столбцов
        self.table.setHorizontalHeaderLabels(['№', 'Имя', 'Тип','Описание','Кол-во'])
        for row_number, row_data in enumerate(rows):
            self.table.insertRow(row_number)
        for column_number, data in enumerate(row_data):
            self.table.setItem(row_number, column_number,QtWidgets.QTableWidgetItem(str(data)))
            self.setCentralWidget(self.table)
        self.query = f"SELECT * FROM Pantry WHERE P_Number LIKE '%{Number}%';"
    ...
    def search(self):
        ...
        self.table.setRowCount(0)
        # Устанавливаем заголовки
        self.c.execute("SELECT * FROM Pantry WHERE P_Number LIKE ? OR P_Name LIKE ? OR P_Type LIKE ? OR P_des LIKE ? OR P_qua LIKE ?;", ('%' + Number + '%','%' + Number + '%','%' + Number + '%','%' + Number + '%','%' + Number + '%',))
        rows = self.c.fetchall()
        # Устанавливаем количество строк и столбцов
        self.table.setHorizontalHeaderLabels(['№', 'Имя', 'Тип','Описание','Кол-во'])
        for row_number, row_data in enumerate(rows):
    ...
    def updata(self):
        ...
        Qua = self.T_B5.text()
        if len(Name) > 0:
            self.c.execute("UPDATE Pantry SET P_Name=? WHERE P_Number=?", (Name, Number))
        if len(Type) > 0:
            self.c.execute("UPDATE Pantry SET P_Type=? WHERE P_Number=?", (Type, Number))
        if len(Des) > 0:
            self.c.execute("UPDATE Pantry SET P_des=? WHERE P_Number=?", (Des, Number))
        if len(Qua) > 0:
            self.c.execute("UPDATE Pantry SET P_qua=? WHERE P_Number=?", (Qua, Number))
        self.conn.commit()
        self.up()
    ...
    def delete(self):
        ...
        if i == 1:
            ...
            # формирование и выполнение SQL запроса
                query = "DELETE FROM Pantry WHERE P_Number=?"
    ...
    def add(self):
        ...
        Qua = self.T_B5.text()
        # формирование и выполнение SQL запроса
        query = "INSERT INTO Pantry(P_Name,P_Type,P_des,P_qua) VALUES (?, ?, ?, ?)"
        self.c.execute(query, (Name, Type, Des, Qua))
        self.conn.commit()
        self.up()
        if self.rb1.isChecked():
            self.clear()
    ...
    def Up1(self):
        ...
        if Number != 0:
            query = f"UPDATE Pantry SET P_qua = P_qua + 1 WHERE P_Number = {Number}"
